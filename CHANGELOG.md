...
- `moduleManager.js` â€” lifecycle manager with per-module init/destroy, error isolation, and state tracking (`unloaded | loading | active | disabled | error`)
- `storage.js` â€” unified `chrome.storage.local` API with `localStorage` fallback for dashboard context; all keys namespaced `btt_`
- `bbcodeParser.js` â€” Bitcointalk-accurate BBCode â†’ HTML converter with XSS hardening (HTML escaped first, code blocks protected as placeholders, `safeUrl`/`safeColor`/`safeFontSize` on all attribute values)
- `sanitizer.js` â€” `safeUrl()` decodes HTML entities and strips control characters before scheme validation to prevent `&amp;javascript:` bypass
- `sharedUI.js` â€” `Toast`, `showModal`, `confirmDialog`, `copyToClipboard`, `downloadFile` shared across modules
- `constants.js` â€” `DEFAULT_SETTINGS`, `BTC_RANKS`, `BTT_STORAGE_PREFIX`
- Floating FAB launcher (â‚¿ button) with quick-access mini menu on every Bitcointalk page

**Layout & Reading modules**
- `darkMode` â€” dark theme toggled via FAB menu and dashboard; no flash on load
- `codeCopyFixer` â€” copy buttons on all `<pre>` code blocks; bypasses Bitcointalk's broken selection
- `navigationBooster` â€” floating â†‘â†“ jump buttons and keyboard shortcut to reply box
- `quoteAssistant` â€” select any text and quote it directly into the reply textarea
- `localDraftSaver` â€” auto-saves textarea content per thread; restores on revisit
- `boardCleaner` â€” hide avatars, signatures; compact post density mode
- `longQuoteCollapser` â€” collapses nested quotes taller than 200px
- `imageCollapser` â€” collapses large images; click to expand
- `mobileEnhancer` â€” viewport fixes and touch-friendly tap targets
- `keywordAlert` â€” highlights user-defined keywords across posts
- `ignoreEnhancer` â€” dims posts from locally ignored users without requiring a page reload

**Security modules**
- `clipboardSafety` â€” warns when a copied crypto address is different from what was shown on screen
- `addressHighlighter` â€” highlights BTC, ETH, LTC, DOGE, TRX addresses in posts; click to copy
- `antiPhishingLinkChecker` â€” marks links pointing to domains not matching the displayed text
- `externalLinkPreview` â€” shows destination domain in a tooltip on hover for external links
- `txidHelper` â€” detects and highlights transaction IDs; click to copy
...
- `adapter.js` â€” `GM_API` shim implementing `GM_getValue`, `GM_setValue`, `GM_deleteValue`, `GM_listValues`, `GM_xmlhttpRequest`, `GM_setClipboard`, `GM_notification`, `GM_log` using `chrome.storage.local`
- `legacyRegistry.js` â€” module manifest for registered legacy scripts
- `exampleLegacyWrapper.js` â€” template for wrapping existing scripts
...
- `bbcodeParser.js` â€” removed `onerror` inline JS event handler from `<img>` tags (blocked by extension CSP); replaced with `class="btt-img-safe"` + CSS rule
- `sanitizer.js` `safeUrl()` â€” decodes HTML entities and strips ASCII control characters before scheme validation
- `storage.js` `importSettings()` â€” validates `enabledModules` is an array of alphanumeric strings (max 100); drops invalid `watchedKeywords`/`ignoredUsers`
- `content.js` â€” removed redundant dynamic imports inside `toggleDarkMode`/`toggleCompactMode`; restored `Toast` import
- `dashboard.js` â€” `parseThreadPage` and `findDuplicates` loaded via dynamic import with fallback; module load failure no longer crashes the entire dashboard
