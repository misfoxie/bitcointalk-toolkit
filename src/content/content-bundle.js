...
Your clipboard may have been modified by malware. Verify manually!`
  ...
  var CACHE_PREFIX = "btt_self_post_results_";
  ...
  function getPostTarget(posterCell) {
    const row1 = posterCell?.closest("tr");
    const row2 = row1?.nextElementSibling;
    const post = row2?.querySelector('[id^="msg_"], [id^="msg"]') || row1?.querySelector('[id^="msg_"], [id^="msg"]') || row2?.querySelector("td.td_headerandpost") || row1?.querySelector("td.td_headerandpost");
  ...
  function getThreadPageUrls(threadInfo, doc = document) {
    ...
    const positiveDiffs = sortedStarts.map((start, index) => index ? start - sortedStarts[index - 1] : 0).filter((diff) => diff > 0);
  ...
  function findPostContainerFromMessage(messageEl) {
    return messageEl?.closest("td.td_headerandpost") || messageEl?.closest("td") || messageEl;
  }
  ...
  function getPostDate2(container) {
  ...
  function detectLoggedInUsername() {
    const saved = document.querySelector("#hellomember b")?.textContent?.trim() || document.querySelector('#hellomember a[href*="action=profile"]')?.textContent?.trim();
    if (saved) return saved;
    const profileLinks = [...document.querySelectorAll('a[href*="action=profile"][href*="u="]')].filter((link) => !link.closest("td.poster_info, .poster_info"));
  ...
  var selfPostFinder_default = {
    ...
    _isSelfPostFinderMutation(mutation) {
      const isOwnNode = (node) => {
        ...
        return Boolean(element?.closest?.(`#${NAV_ID}, #${STYLE_ID3}`));
      };
    ...
    _addNav() {
      ...
      <label style="display:block;margin-bottom:5px">
        Username
        <input type="text" data-role="username" placeholder="Username" value="${escapeHtml2(this._username)}">
      </label>
    ...
    async _scanWholeThread(forceRefresh = false) {
      ...
      for (let i = 0; i < pages.length; i += 1) {
        ...
        try {
          const html = canUseCurrentDocument && page.start === threadInfo.start || !canUseCurrentDocument && pages.length === 1 ? document.documentElement.outerHTML : await fetchThreadPage(page.url);
        ...
        if (i < pages.length - 1) await sleep2(REQUEST_DELAY_MS);
      }
    ...
    _renderSelfPostResults(results, username, warnings = []) {
      ...
      if (!target) return;
      const safeUser = escapeHtml2(username);
      const warningsHtml = warnings.length ? `<div style="color:#fbbf24;margin-top:7px">${warnings.map(escapeHtml2).join("<br>")}</div>` : "";
      if (!results.length) {
        target.innerHTML = `
      ...
      <details open>
        ...
        ${results.map((post) => `
          <div class="btt-spf-result">
            <div><strong>Page ${post.pageNumber}</strong> <a href="${escapeHtml2(post.link)}">${escapeHtml2(post.postNumber)}</a></div>
            ${post.date ? `<div class="btt-spf-muted">${escapeHtml2(post.date)}</div>` : ""}
            <div class="btt-spf-preview">${escapeHtml2(post.preview || "No preview available.")}</div>
            <a class="btt-spf-button" href="${escapeHtml2(post.link)}">Open post</a>
          </div>
        `).join("")}
      </details>
    `;
...
<div style="display:flex;flex-direction:column;gap:14px;max-width:600px">
  ...
  var DEFAULT_TRUST_VIEW_URL = "https://bitcointalk.org/index.php?action=trust;dtview";
  ...
  var NORMALIZED_CACHE_KEY = "btt_dt_normalized_cache";
  ...
  var CUSTOM_DT_MARKER = "BTT_TOOLKIT_DT_DATA_V1";
  var CUSTOM_DT_FETCH_DELAY_MS = 400;
  var POSITIONS = {
  ...
  function normalizeDtData(raw, sourceUrl = "") {
    ...
    normalized.removedUsers.forEach((user) => {
      const key = user.userId || user.user_id || (user.username ? `user:${normalizeUsername2(user.username)}` : "");
      if (key) normalized.removed_index[key] = true;
      if (user.username) normalized.removed_index[normalizeUsername2(user.username)] = true;
    });
    ...
    for (const [key, user] of Object.entries(sourceUsers)) {
      ...
      const cacheKey = userId || (username ? `user:${normalizeUsername2(username)}` : `user:${normalizeUsername2(key)}`);
      if (!cacheKey || !/^DT[1-9]\d*$/.test(dtLevel)) continue;
      if (normalized.removed_index[cacheKey] || normalized.removed_index[normalizeUsername2(username)]) continue;
      ...
      if (username) {
        ...
        normalized.username_index[normalizeUsername2(username)] = cacheKey;
      }
    }
    ...
    if (sourceIndex && typeof sourceIndex === "object") {
      for (const [username, key] of Object.entries(sourceIndex)) {
        if (normalized.users[String(key)]) {
          ...
          normalized.username_index[normalizeUsername2(username)] = String(key);
        }
      }
    }
  ...
  function isCacheValidForBadges(data) {
    ...
    return Boolean(updatedAt) && isCacheFresh({ updated_at: updatedAt }) && (sourceText.includes(DEFAULT_TRUST_URL) || sourceIsBitcointalk) && userEntries.length >= MIN_VALID_CACHE_USERS && (sourceIsBitcointalk || numericUidKeys) && (data?.stats?.dt1_count || 0) > 0 && (data?.stats?.dt2_count || 0) > 0;
  }
  function delay(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
  }
  ...
  function discoverTopicPageUrls(html, threadUrl) {
    ...
    const positiveDiffs = sorted.map((start, index) => index ? start - sorted[index - 1] : 0).filter((diff) => diff > 0);
    const detectedStep = positiveDiffs.length ? Math.min(...positiveDiffs) : 20;
    const step = detectedStep > 20 ? 20 : detectedStep;
    ...
    return [...starts].filter(Number.isFinite).sort((a, b) => a - b).map((start) => makeTopicPageUrl(topicId, start));
  }
  ...
  function extractAuthorFromPost(post) {
    ...
    const link = posterArea.querySelector('b > a[href*="action=profile"][href*="u="]') || posterArea.querySelector('a[href*="action=profile"][href*="u="]') || post.querySelector('td.poster_info a[href*="action=profile"][href*="u="], .poster_info a[href*="action=profile"][href*="u="]');
    return {
  ...
  function authorMatchesFilter(author, authorFilter) {
    ...
    const wantedUsername = wantedUid ? "" : normalizeUsername2(cleanFilter);
    return Boolean(
      wantedUid && String(author?.uid || "") === wantedUid || wantedUsername && normalizeUsername2(author?.username || "") === wantedUsername
    );
  }
  function extractCodeBlocksFromPost(post) {
    const blocks = Array.from(post.querySelectorAll(".code, code, pre, textarea, td, div")).map((el) => getElementTextWithBreaks(el)).filter((text) => text && text.includes(CUSTOM_DT_MARKER));
  ...
  function extractNearbyMsgIdFromHtml(htmlBefore) {
    ...
    return getLastRegexMatch(text, /name=["']msg(\d+)["']/gi) || getLastRegexMatch(text, /id=["']msg(\d+)["']/gi) || getLastRegexMatch(text, /[.#]msg(\d+)/gi);
  }
  ...
  function collectCustomDtHtmlCodeCandidates(html, sourceUrl, authorFilter = "", topicId = "") {
    ...
    while (match = codeRegex.exec(text)) {
      ...
      const contextStart = Math.max(0, match.index - 16e3);
      ...
      data.fetch_status = data.source_msg_id ? `Using custom Bitcointalk DT post: msg${data.source_msg_id}` : "Using custom Bitcointalk DT post data";
  ...
  function parseSimpleDtList(text, sourceUrl) {
    const normalizedText = String(text || "").replace(/\[\/?code\]/gi, "\n").replace(CUSTOM_DT_MARKER, `
${CUSTOM_DT_MARKER}
`).replace(/\b(DT1|DT2|REMOVED|EXCLUDED)\s*:/gi, "\n$1:\n");
    ...
    for (const line of lines) {
      if (/^DT1\s*:?$/i.test(line)) {
        section = "DT1";
        continue;
      }
      if (/^DT2\s*:?$/i.test(line)) {
        section = "DT2";
        continue;
      }
      if (/^(REMOVED|EXCLUDED)\s*:?$/i.test(line)) {
        section = "REMOVED";
        continue;
      }
      ...
      const key = uid || `user:${normalizeUsername2(username)}`;
      if (removed) {
        removedUsers.push({
          userId: uid || null,
          user_id: uid || null,
    ...
    for (const [key, user] of Object.entries(users)) {
      ...
      if (user.username) {
        ...
        data.username_index[normalizeUsername2(user.username)] = key;
      }
    }
    removedUsers.forEach((user) => {
      if (user.user_id) data.removed_index[user.user_id] = true;
      if (user.username) data.removed_index[normalizeUsername2(user.username)] = true;
    });
    recomputeStats(data);
  ...
  function parseCustomDtPayload(text, sourceUrl) {
    ...
    if (parsedJson) {
      const data2 = normalizeDtData(parsedJson, sourceUrl);
      data2.updated_at = parsedJson.updated_at || parsedJson.updatedAt || (/* @__PURE__ */ new Date()).toISOString();
      data2.updatedAt = data2.updated_at;
      data2.source = [sourceUrl];
      data2.source_url = sourceUrl;
      data2.sourceUrl = sourceUrl;
      data2.fetch_status = "Using custom Bitcointalk DT thread data";
      recomputeStats(data2);
      if (data2.stats.total_users > 0) return data2;
    }
  ...
  async function saveNormalizedDtCache(data, fetchStatus = "") {
    ...
    normalized.updated_at = normalized.updated_at || (/* @__PURE__ */ new Date()).toISOString();
...
`;
  ...
  function createLauncher() {
    ...
    items.forEach(({ icon, label, action }) => {
      const btn = document.createElement("button");
      btn.innerHTML = `<span>${escapeHtml3(icon)}</span> ${escapeHtml3(label)}`;
      btn.addEventListener("click", () => {
        menu.classList.remove("open");
        action();
      });
  ...
  function buildForumThemeColorControls(colors) {
    ...
    <section class="btt-theme-group">
      <h4>${escapeHtml3(group.label)}</h4>
      <div class="btt-theme-color-grid">
        ${group.fields.map(([key, label]) => {
      const value = normalized[key] || DEFAULT_FORUM_CUSTOM_COLORS[key];
      return `
            <label class="btt-theme-color-row" data-theme-color-row="${escapeHtml3(key)}">
              <span>${escapeHtml3(label)}</span>
              <input type="color" value="${escapeHtml3(value)}" data-theme-color="${escapeHtml3(key)}">
              <input type="text" value="${escapeHtml3(value)}" maxlength="7" data-theme-hex="${escapeHtml3(key)}" spellcheck="false">
              <small data-theme-rgb="${escapeHtml3(key)}">${escapeHtml3(hexToRgbText(value))}</small>
            </label>
          `;
    }).join("")}
      </div>
  ...
  async function openForumThemePanel() {
    ...
    <div class="btt-forum-theme-panel" role="dialog" aria-modal="true" aria-label="Forum Theme">
      <div class="btt-forum-theme-header">
        <div>
          <h3>Forum Theme</h3>
          <p>Customize Bitcointalk page colors. Current: <strong data-current-skin>${escapeHtml3(settings.forumSkinName || (activeSkin === "original" ? "Default Bitcointalk" : "Custom Theme"))}</strong></p>
        </div>
      ...
      <section class="btt-theme-group">
        ...
        <div class="btt-theme-skin-grid">
          ${FORUM_SKIN_PRESETS.map((skin) => `
            <button type="button" class="${skin.id === activeSkin ? "active" : ""}" data-skin-id="${escapeHtml3(skin.id)}">
              ${escapeHtml3(skin.name)}
            </button>
          `).join("")}
        </div>
      </section>
